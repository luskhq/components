...
class Dropdown extends React.Component {
  ...
  render() {
    ...
    return (
      <span className={c(s.position("relative"), s.display("inline-block"))}>
        {this.renderHandler()}
        {open && (
          <DropdownMenu
            alignMenu={alignMenu}
  ...
  renderHandler() {
    const { type, alignMenu: _omit, ...passingProps } = this.props;
    switch (type) {
      case "flat":
        return (
          <FlatButton
    ...
    return (
      <div
        ...
        className={c(
          ...
          alignMenu === "left" && [s.left("0"), s.transformOrigin("top-left")],
          alignMenu === "right" && [
            s.right("0"),
